...
def _entradas():
    ...
    with st.container(key="dashcard_hip_datos"):
        st.subheader("Datos del crédito")
        f1, f2, f3 = st.columns(3)
        with f1:
            vivienda = input_pesos("Valor de la vivienda", "hip_vivienda",
                                   205_000_000)
        with f2:
        ...
        fecha_inicio = g1.date_input(
            ...
            help="Mes en que empiezas a pagar. Define cuándo caen "
                 "prima/cesantías y el mes de finalización.")
        plazo_anios = g2.number_input("Plazo (años)", 1, 40, 20, step=1)
        tasa_ea = g3.number_input(
            "Tasa de interés efectiva anual (%)", 0.0, 60.0, 11.0,
            step=0.1) / 100

    n_meses = int(plazo_anios) * 12
    with st.container(key="dashcard_hip_abonos"):
        ...
        with e2:
            arriendo = input_pesos("🏘️ Arriendo mensual", "hip_arriendo",
                                   700_000,
        ...
        with e3:
            ...
            prima_mes = st.selectbox(
                "Mes de la prima", list(MESES_NOMBRE.keys()),
                index=5, format_func=lambda m: MESES_NOMBRE[m])
        with e4:
            cesantias = input_pesos(
                "🏦 Cesantías anuales", "hip_cesantias", 0,
...
def render():
    ...
    hay_abonos = (p["abono"] > 0
                  or (p["arriendo"] > 0 and p["arriendo_meses"] > 0)
...
def _impacto(p, cuota, int_base, int_ab, meses_base, meses_ab,
             fin_base, fin_ab, total_abonado):
    with st.container(key="dashcard_hip_impacto"):
        ...
        a1.metric("🏁 Termina (con abonos)", fecha_larga(*fin_ab),
                  delta=f"-{meses_a_texto(meses_ahorrados)}",
                  delta_color="inverse",
        ...
        st.success(
            f"Con {', '.join(detalle)}, terminarías en "
            f"**{fecha_larga(*fin_ab)}** ({meses_a_texto(meses_ab)}) en vez de "
            f"{fecha_larga(*fin_base)} — te ahorras "
            f"**{meses_a_texto(meses_ahorrados)}** y dejas de pagar "
...
def _grafica_saldo(filas_base, filas_ab, hay_abonos):
    with st.container(key="dashcard_hip_saldo"):
...
def _tabla_amortizacion(filas):
    with st.container(key="dashcard_hip_tabla"):
