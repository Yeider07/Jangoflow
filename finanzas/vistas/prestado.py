...
def render():
    ...
    with st.container(key="dashcard_prest_main"):
        st.subheader("🤝 Dinero prestado (global)")
        st.caption(
            "Esta sección NO va por mes: es tu lista global de préstamos, vive "
            "hasta que te paguen. **Total prestado**: cuánto le prestaste a "
            "cada quien. **Devuelto**: lo que ya te han devuelto. El pendiente "
            "por cobrar se descuenta de tu ahorro líquido en el General.")
        ...
        with st.expander(f"✅ Préstamos ya pagados ({len(df_pag)})",
                         expanded=False):
            if df_pag.empty:
        ...
        if not editado.empty:
            ...
            tot_d = float(pd.to_numeric(editado["devuelto"],
                                        errors="coerce").sum())
        ...
        if st.button("💾 Guardar préstamos"):
            db.guardar_prestamos(
                editado.drop(columns=["pendiente"], errors="ignore"))
            st.session_state.pop("ed_prest_act", None)
            st.session_state.pop("ed_prest_pag", None)
            st.success("Préstamos guardados.")
            st.rerun()

        st.info("💡 Cuando te devuelvan, sube el **Devuelto**. Al quedar "
                "Devuelto = Total prestado, el préstamo se mueve solo a "
                "**✅ Ya pagados** y desaparece de esta lista.")

    _resumen_por_persona(editado)


def _resumen_por_persona(editado):
    with st.container(key="dashcard_prest_resumen"):
